...
public class ArrowArrayColumn implements IColumn{
  ...
  public ArrowArrayColumn( final String columnName , final ListVector vector ){
    ...
    spread = new Spread();
    spread.addColumn( ArrowColumnFactory.convert( childVector.getField().getName() , childVector ) );
    ...
    for( int i = 0 ; i < cellArray.length ; i++ ){
      if( vector.isNull(i) ){
        ...
        cellArray[i] = new ArrayCell( new SpreadArrayLink( spread , i , currentStart , currentStart + length ) );
        currentStart += length;
      }
    }
  }
  ...
  @Override
  public void addCell( final ColumnType type , final ICell obj , final int index ) throws IOException{
    throw new UnsupportedOperationException( "This column is read only." );
  }
