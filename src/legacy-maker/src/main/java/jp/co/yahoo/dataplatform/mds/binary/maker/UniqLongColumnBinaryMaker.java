...
public class UniqLongColumnBinaryMaker implements IColumnBinaryMaker{
  ...
  public ColumnBinary toBinary(final ColumnBinaryMakerConfig commonConfig , final ColumnBinaryMakerCustomConfigNode currentConfigNode , final IColumn column ) throws IOException{
    ...
    if( ! hasNull && dicMap.size() == 2 ){
      ...
      dicBuffer.getLong();
      return ConstantColumnBinaryMaker.createColumnBinary( new LongObj( dicBuffer.getLong() ) , column.getColumnName() , column.size() );
    }
  ...
  public int calcBinarySize( final IColumnAnalizeResult analizeResult ){
    ...
    int dicSize = ( analizeResult.getUniqCount() + 1 ) * Long.BYTES;
  ...
  public class LongColumnManager implements IColumnManager{
    ...
    public LongColumnManager( final ColumnBinary columnBinary , final int columnBinaryStart , final int columnBinaryLength ) throws IOException{
      this.columnBinary = columnBinary;
    ...
    private void create() throws IOException{
      ...
      LongBuffer dicBuffer = ByteBuffer.wrap( decompressBuffer , dicStart , dicSize ).asLongBuffer();
      dicBuffer.get();
      PrimitiveObject[] dicArray = new PrimitiveObject[dicSize/Long.BYTES];
      for( int i = 1 ; i < dicArray.length ; i++ ){
        dicArray[i] = new LongObj( dicBuffer.get() );
      }
