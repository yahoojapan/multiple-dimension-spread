...
public class DumpBytesColumnBinaryMaker implements IColumnBinaryMaker{
  ...
  public ColumnBinary toBinary(final ColumnBinaryMakerConfig commonConfig , final ColumnBinaryMakerCustomConfigNode currentConfigNode , final IColumn column , final MakerCache makerCache ) throws IOException{
    ...
    for( int i = 0 ; i < column.size() ; i++ ){
      ...
      byte[] obj = bytesCell.getRow().getBytes();
  ...
  public class BytesDicManager implements IDicManager{
    ...
    public BytesDicManager( final ICompressor compressor , final IPrimitiveObjectConnector primitiveObjectConnector , final byte[] dicBinary , final int dicStart , final int dicLength ){
    ...
    private void decompress() throws IOException{
      ...
      for( byte[] data : BinaryDump.binaryToBytesList( dicBinary , dicStart , dicLength ) ){
        dicList.add( primitiveObjectConnector.convert( PrimitiveType.BYTES , new BytesObj( data ) ) );
      }
      isSet = true;
    }
  ...
  public class BytesColumnManager implements IColumnManager{
    ...
    private void create() throws IOException{
      ...
      List<Integer> columnIndexList = SortedIntegerConverter.getIntegerList( binary , columnBinaryStart , columnBinaryLength );
      column = new PrimitiveColumn( ColumnType.BYTES , columnBinary.columnName );
      IDicManager dicManager = new BytesDicManager( compressor , primitiveObjectConnector , binary , dicBinaryStart , dicBinaryLength );
      int dicIndex = 0;
      for( Integer index : columnIndexList ){
        column.addCell( columnBinary.columnType , new LazyCell( ColumnType.BYTES , dicManager , dicIndex ) , index );
        dicIndex++;
      }
