...
public class UnsafeRangeDumpDoubleColumnBinaryMaker implements IColumnBinaryMaker{
  ...
  public int calcBinarySize( final IColumnAnalizeResult analizeResult ){
    if( analizeResult.getNullCount() == 0 && analizeResult.getUniqCount() == 1 ){
      ...
      return analizeResult.getColumnSize() + ( analizeResult.getColumnSize() - analizeResult.getNullCount() ) * Double.BYTES;
    }
  }
  ...
  public void loadInMemoryStorage( final ColumnBinary columnBinary , final IMemoryAllocator allocator ) throws IOException{
    ...
    if( type == (byte)1 ){
      ...
      IReadSupporter doubleReader = ByteBufferSupporterFactory.createReadSupporter( binary , columnBinary.rowCount + Byte.BYTES , Double.BYTES * columnBinary.rowCount , order );
      ...
      IReadSupporter doubleReader = ByteBufferSupporterFactory.createReadSupporter( binary , Byte.BYTES , Double.BYTES * columnBinary.rowCount , order );
      for( int i = 0 ; i < columnBinary.rowCount ; i++ ){
        allocator.setDouble( i , doubleReader.getDouble() );
      }
    }
