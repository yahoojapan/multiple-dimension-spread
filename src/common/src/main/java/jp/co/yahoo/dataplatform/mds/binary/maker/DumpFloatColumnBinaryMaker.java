...
public class DumpFloatColumnBinaryMaker implements IColumnBinaryMaker{
  ...
  public ColumnBinary toBinary(final ColumnBinaryMakerConfig commonConfig , final ColumnBinaryMakerCustomConfigNode currentConfigNode , final IColumn column , final MakerCache makerCache ) throws IOException{
    ...
    for( int i = 0 ; i < column.size() ; i++ ){
      ...
      objList.add( Float.valueOf( byteCell.getRow().getFloat() ) );
  ...
  public byte[] convertBinary( final List<Integer> columnList , final List<Float> objList , ColumnBinaryMakerConfig currentConfig ) throws IOException{
    byte[] columnBinaryRaw = BinaryUtil.toLengthBytesBinary( SortedIntegerConverter.getBinary( columnList ) );
    byte[] dicRawBinary = BinaryUtil.toLengthBytesBinary( BinaryDump.dumpFloat( objList ) );
  ...
  public class FloatDicManager implements IDicManager{
    ...
    public FloatDicManager( final IPrimitiveObjectConnector primitiveObjectConnector , final FloatBuffer dicBuffer ){
    ...
    public PrimitiveObject get( final int index ) throws IOException{
      ...
      return primitiveObjectConnector.convert( PrimitiveType.FLOAT , new FloatObj( dicBuffer.get( index - 1 ) ) );
    }
  ...
  public class FloatColumnManager implements IColumnManager{
    ...
    private void create() throws IOException{
      ...
      List<Integer> columnIndexList = SortedIntegerConverter.getIntegerList( binary , columnBinaryStart , columnBinaryLength );
      IDicManager dicManager = new FloatDicManager( primitiveObjectConnector ,  ByteBuffer.wrap( binary , dicBinaryStart , dicBinaryLength ).asFloatBuffer() );
