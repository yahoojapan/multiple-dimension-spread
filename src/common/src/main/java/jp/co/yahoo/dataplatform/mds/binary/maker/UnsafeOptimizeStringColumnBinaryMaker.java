...
public class UnsafeOptimizeStringColumnBinaryMaker implements IColumnBinaryMaker{
  ...
  public interface IDictionaryIndexMaker{
    ...
    IntBuffer getIndexIntBuffer( final int size , final byte[] buffer , final int start , final int length ) throws IOException;
  ...
  public static class ByteDictionaryIndexMaker implements IDictionaryIndexMaker{
    ...
    @Override
    public IntBuffer getIndexIntBuffer( final int size , final byte[] buffer , final int start , final int length ) throws IOException{
      ByteBuffer wrapBuffer = ByteBuffer.wrap( buffer , start , length );
  ...
  public ColumnBinary toBinary(final ColumnBinaryMakerConfig commonConfig , final ColumnBinaryMakerCustomConfigNode currentConfigNode , final IColumn column ) throws IOException{
    ...
    indexMaker.create( indexArray , binaryRaw , Integer.BYTES * 2 , indexBinaryLength );

    ByteBuffer lengthBuffer = ByteBuffer.wrap( binaryRaw , Integer.BYTES * 2 + indexBinaryLength , lengthBinaryLength );
    lengthMaker.create( stringList , lengthBuffer );
    wrapBuffer.position( Integer.BYTES * 2 + indexBinaryLength + lengthBinaryLength );
  ...
  public void loadInMemoryStorage( final ColumnBinary columnBinary , final IMemoryAllocator allocator ) throws IOException{
    ...
    IntBuffer indexBuffer = indexMaker.getIndexIntBuffer( columnBinary.rowCount , binary , Integer.BYTES * 2 , indexBinaryLength );
    ByteBuffer lengthBuffer = ByteBuffer.wrap( binary , Integer.BYTES * 2 + indexBinaryLength , lengthBinaryLength );
    int[] lengthArray = lengthMaker.getLengthArray( lengthBuffer , columnBinary.cardinality );

    int currentStart = Integer.BYTES * 2 + indexBinaryLength + lengthBinaryLength;
